export const site = {
  name: 'Cindy',
  role: '營養 × 教育 × 資訊',
  headline: '把故事、健康與科技做成溫暖又好玩的作品',
  location: 'Taiwan',
  email: 'hello@example.com',
  social: [
    { label: 'GitHub', href: 'https://github.com/yourname' },
    { label: 'LinkedIn', href: 'https://linkedin.com/in/yourname' }
  ],
  heroCTA: [
    { label: '聯絡我', href: '#contact' },
    { label: '下載履歷', href: '#', outline: true }
  ],
  about: {
    intro: '營養師 | 兒童程式教師 | 跨領域魔法師',
    highlights: [
      '兒童程式教學設計與教材開發',
      'AI × 網站 / APP / 自動化'
    ]
  },
  projects: [
    {
      title: 'Re:Memory 語憶心聲',
      bg: '#BA8A4D',
      description: '跨代故事平台：AI 小光對話、故事生成與生命地圖。',
      tags: ['Vue', 'Django', 'AI', 'GCP'],
      link: '#'
    },
    {
      title: 'AI 排班平台',
      bg: '#283926',
      description: 'LSTM 模型，自動產生便利商店排班。',
      tags: ['Python', 'PyTorch', 'Autoencoder'],
      link: '#'
    },
    {
      title: '營養素計算APP',
      bg: '#111111',
      description: 'BMR計算+每日活動評估。',
      tags: ['營養', '教育', '遊戲'],
      link: '#'
    },
    {
      title: '萌寵猜拳王',
      bg: '#BA8A4D',
      description: '簡易遊戲搭配可愛的故事劇情',
      tags: ['Vue', 'Django', 'AI', 'GCP'],
      link: '#'
    },
    {
      title: '產品到期自動提醒平台',
      bg: '#283926',
      description: '串聯雲端Google Excel，每日自動進行產品到期提醒。',
      tags: ['Python', 'PyTorch', 'Autoencoder'],
      link: '#'
    },
    {
      title: '自動查找資料平台',
      bg: '#111111',
      description: '每日自動爬取指定網站資料，並整理成報表。',
      tags: ['營養', '教育', '遊戲'],
      link: '#'
    },
    {
      title: '教學素材平台',
      bg: '#111111',
      description: '提供各式免費的網路去背圖片及學習題庫。',
      tags: ['營養', '教育', '遊戲'],
      link: '#'
    }
  ],
  experience: [
    { time: '2025 — Now', title: '網站設計', detail: '視覺與互動樣式、流程圖。' },
    { time: '2025畢業', title: '輔大AI專班', detail: '機器學習、深度學習、Pythont程式設計、文字探勘及大數據分析。' },
    { time: '2021 — Now', title: '兒童程式教師', detail: '互動遊戲教材、程式邏輯、Scratch/Python/JS 課程。' },
    { time: '2021 — Now', title: '營養師', detail: '營養教育、飲食諮詢、課程內容設計。' },
    { time: '2017畢業', title: '輔大營養系', detail: '膳食療養、團體膳食、營養評估及營養教育學習。' }
  ],
  skills: ['Vue 3', 'Tailwind CSS', 'Python', 'Django', 'PyTorch', 'Pandas', 'Docker', 'GCP', '營養師','ITS Python','兒童程式教師','AI 應用']
}

